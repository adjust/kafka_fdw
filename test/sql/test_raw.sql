...
DROP FOREIGN TABLE kafka_test_raw;

-- check writing raw data to kafka and reading it back
CREATE FOREIGN TABLE kafka_test_raw (
    part int OPTIONS (partition 'true'),
    offs bigint OPTIONS (offset 'true'),
    message text
...
SERVER kafka_server OPTIONS
    (format 'raw', topic 'contrib_raw', batch_size '30', buffer_delay '100');

INSERT INTO kafka_test_raw (message) VALUES ('123');
SELECT message FROM kafka_test_raw;